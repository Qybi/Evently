...
// flagging TRequest as IBaseCommand to ensure that only commands are validated, not queries
internal sealed class ValidationPipelineBehaviour<TRequest, TResponse>(IEnumerable<IValidator<TRequest>> validators) : IPipelineBehavior<TRequest, TResponse> where TRequest : IBaseCommand
{
