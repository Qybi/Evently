...
internal sealed class ProcessInboxJob(EventsDbContext eventsDbContext,
    ...
    ILogger<ProcessInboxJob> logger) : IJob
{
    ...
    public async ValueTask Execute(IJobExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        await using IDbContextTransaction transaction = await eventsDbContext.Database.BeginTransactionAsync(cancellationToken);
        ...
        foreach (InboxMessage inboxMessage in inboxMessages)
        {
            ...
            try
            {
                ...
                IEnumerable<IIntegrationEventHandler> integrationEventHandlers = IntegrationEventHandlersFactory.GetHandlers(
                    ...
                    Events.Presentation.AssemblyReference.Assembly);
    ...
    private async Task<IReadOnlyList<InboxMessage>> GetInboxMessagesAsync(CancellationToken cancellationToken)
    {
        ...
        List<InboxMessage> inboxMessages = await eventsDbContext.Set<InboxMessage>()
            .FromSql($"""
                SELECT *
